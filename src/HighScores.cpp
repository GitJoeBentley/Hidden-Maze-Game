...
Score::Score(const char* n, int sc, int br, int tim, time_t t)
...
std::ostream& operator<<(std::ostream& out, const Score& obj)
{
    out << std::left << std::setw(20) << obj.name
        << std::right << std::setw(7) << obj.score
        << std::right << std::setw(10) << obj.bruises
        << std::right << std::setw(8) << obj.ptime
        << std::setw(13) << obj.getDateAsString() << std::endl;
    return out;
}
...
std::ostream& operator<<(std::ostream& out, const HighScores& scores)
{
    ...
    else
    {
        ...
        out << "     Name                   Score    Bruises   Time      Date" << std::endl;
        auto count = 1;
